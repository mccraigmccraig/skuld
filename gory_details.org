...
Control effects can bypass continuations:
- =Throw= discards the continuation entirely
- =Yield= captures and stores it for later

If cleanup only happened in a wrapped continuation, these control effects would
skip cleanup. We need cleanup to run *always*, in reverse installation order.
