...
The =state= map holds effect state. Keys are typically effect module atoms,
or tuples like ={EffectModule, tag}= to support multiple instances of the same
effect:
...
The key insight: =bind= returns another computation  *function*, not a result.
The computation isn't executed until someone calls it with an environment and
continuation.
...
1. *Interception*: ={Module, pattern} -> body= - intercepts effects via =Module.intercept/2=
2. *Installation*: =Module -> config= - installs handlers via
   =Module.__handle__/2=
...
Key points:
- The tagged pattern ={Throw, :negative}= extracts the module (=Throw=) and inner pattern (=:negative=)
- The handler function receives the thrown value directly (not the tuple)
- Unhandled values are automatically re-thrown (default re-dispatch)
...
Key points:
- Bare module patterns call =Module.__handle__(comp, config)=
- First clause becomes innermost handler, last clause outermost
- The config value (right side of =->=) is passed to =__handle__/2=

*** Complex example: Mixed Throw and Yield interception
...
Expands to (showing structure):

#+begin_src elixir
# Outer layer: Throw interception (catches :timeout and wraps others)
Skuld.Effects.Throw.intercept(
  # Middle layer: Yield interception (responds to :get_config)
  Skuld.Effects.Yield.intercept(
    # Inner: the main computation
    Comp.bind(Yield.yield(:get_config), fn config ->
      Comp.bind(risky_operation(config), fn result ->
        Comp.pure(result)
      end)
    end),
    # Yield handler
    fn __skuld_caught_value__ ->
      case __skuld_caught_value__ do
        :get_config -> Comp.pure(%{default: true})
        __skuld_unhandled__ -> Skuld.Effects.Yield.yield(__skuld_unhandled__)
      end
    end
  ),
  # Throw handler
  fn __skuld_caught_value__ ->
    case __skuld_caught_value__ do
      :timeout -> Comp.pure(:retry_later)
      err -> Skuld.Effects.Throw.throw({:wrapped, err})
    end
  end
)
#+end_src
...
#+begin_src elixir
# Group 3: outer Throw handler (catches :c)
Skuld.Effects.Throw.intercept(
  # Group 2: Yield handler (responds to :x)
  Skuld.Effects.Yield.intercept(
    # Group 1: inner Throw handler (catches :a and :b)
    Skuld.Effects.Throw.intercept(
      computation(),
      fn val ->
        case val do
    ...
    fn val ->
      case val do
        :x -> handle_x()
        other -> Skuld.Effects.Yield.yield(other)
      end
    end
  ),
  fn val ->
...
This grouping enables control over error propagation: a throw from the Yield
handler (group 2) would be caught by group 3, not group 1.
...
comp do
  ...
  {Throw, :recoverable} -> :fallback           # Interception
...
#+begin_src elixir
# Outermost: Throw handler installation
Skuld.Effects.Throw.__handle__(
  # Middle: State handler installation
  Skuld.Effects.State.__handle__(
    # Inner: Throw interception
    Skuld.Effects.Throw.intercept(
      Comp.bind(risky_operation(), fn result -> Comp.pure(result) end),
      fn val ->
        case val do
          :recoverable -> Comp.pure(:fallback)
          other -> Skuld.Effects.Throw.throw(other)
        end
      end
    ),
...
The interception (={Throw, :recoverable}=) catches locally, while the installation
(=Throw -> nil=) provides the outer handler that processes uncaught throws.
...
Installs a handler with setup and cleanup. This is the key mechanism that makes
Skuld's scoped handlers tractable - it ensures cleanup happens in reverse
installation order regardless of whether the computation completes normally
or throws. (Suspend defers cleanup until the computation resumes and completes.)

*** The problem

=Throw= discards the continuation entirely - it never gets called. If cleanup
only happened in a wrapped continuation, Throw would skip cleanup entirely.

(=Yield= is different: it captures the continuation for later. The wrapped
continuation WILL be called when resumed, so cleanup happens then. Yield
doesn't skip cleanup - it defers it.)

*** The solution: dual cleanup paths

=scoped= installs cleanup in TWO places:

1. *Wrapped continuation* (=normal_k=): For normal completion (and resumed Suspends)
...
*Throw* (comp discards continuation - terminal):
...
*Suspend* (comp captures continuation for later - non-terminal):
1. =comp= returns =%Suspend{}= with =normal_k= captured inside
2. =Comp.run= sees the sentinel and calls =ISentinel.run=
3. =ISentinel.run= applies =transform_suspend= to decorate (expose state on Suspend.data)
4. =leave_scope= is NOT called - computation is paused, not finished
5. Later, caller invokes =resume.(input)=
6. Resume calls the captured =normal_k=, computation continues
7. When computation finally completes (normal or Throw), =finally_k= runs then

The key distinction: =leave_scope= only runs for *terminal* cases - when the
computation is truly finished and the continuation has been discarded. Suspend
is non-terminal; the continuation is captured for later, so cleanup is deferred
until the resumed computation completes.

*** Reverse order guarantee

Because each =scoped= call:
- Saves the current =leave_scope= as =previous_leave_scope=
- Installs its own =my_leave_scope= that chains to =previous_leave_scope=

Cleanup happens in reverse installation order:
...
This matches the intuition from try/finally blocks - inner resources clean up
before outer ones.
...
=ISentinel= (I for "interface") is a protocol that determines how
...
For these cases, consider =FxFasterList=, or Yield-based iterations, or
just write pure code with accumulators and context parameters.
