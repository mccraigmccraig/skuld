...
1. *Wrapped continuation* (=normal_k=): For normal completion
2. *leave_scope chain*: For abnormal exits (Throw, Suspend)
...
2. =Comp.run= sees the sentinel, applies =transform_suspend=, returns
3. Later, caller invokes =resume.(input)=
4. Resume calls the captured =normal_k=
5. =normal_k= runs =finally_k= for cleanup (when resumed)
