defmodule Skuld.Effects.Port.Contract do
  ...
  A Port contract defines a set of operations, generating:
    ...
    * **Behaviour** (`X.Behaviour`) — plain Elixir callbacks for
      non-effectful implementations (generated by HexPort)
    * **Port** (`X.Port`) — plain dispatch facade via config or
      test handlers (generated by HexPort)
    * **Effectful** (`X.Effectful`) — computation-returning
      callbacks for effectful implementations (generated by Skuld)
    * **EffectPort** (`X.EffectPort`) — effectful dispatch facade with typed
      public functions returning `computation(return_type)`, bang variants
      (unwrap or throw), and key helpers for test stubs (generated by Skuld)
    * **Introspection** — `__port_operations__/0` on the contract module
      (generated by HexPort)
  ...
  The `MyContract.EffectPort` module's caller functions satisfy the Effectful
  behaviour (they return computations that emit Port effects).
  ...
  This generates:
      ...
      # Effectful dispatch facade (generated by Skuld)
      MyApp.Repository.EffectPort
  ...
  ## Handler Installation
      ...
      MyApp.Repository.EffectPort.get_todo("tenant1", "todo1")
  ...
  defmacro __before_compile__(env) do
    ...
    callers = Enum.map(operations, &generate_caller(&1, contract_module))

    bangs =
      operations
      |> Enum.filter(fn op -> op.bang_mode != :none end)
      |> Enum.map(&generate_bang(&1, contract_module))

    key_helpers = Enum.map(operations, &generate_key_helper(&1, contract_module))

    effect_port_module = Module.concat(env.module, EffectPort)
    ...
    quote do
      defmodule unquote(provider_module) do
        ...
        The `#{inspect(unquote(effect_port_module))}` module's caller
        functions satisfy this behaviour.
        """
      ...
      defmodule unquote(effect_port_module) do
        @moduledoc """
        Effectful dispatch facade for `#{inspect(unquote(env.module))}`.
        ...
        This is the effectful counterpart to `#{inspect(unquote(env.module))}.Port`
        (plain dispatch facade generated by HexPort).
        """
  ...
  defp generate_key_helper(%{name: name, param_names: param_names}, contract_module) do
    param_vars = Enum.map(param_names, fn pname -> {pname, [], nil} end)
    args_list = param_vars
    ...
    doc_string =
      if param_names != [] do
        ...
        "Build a test stub key for the `#{name}` port operation.\n\nUse with `Port.with_test_handler/2`:\n\n    Port.with_test_handler(%{\n      #{inspect(contract_module)}.EffectPort.key(:#{name}, #{param_example}) => {:ok, result}\n    })\n"
      else
        "Build a test stub key for the `#{name}` port operation.\n\nUse with `Port.with_test_handler/2`:\n\n    Port.with_test_handler(%{\n      #{inspect(contract_module)}.EffectPort.key(:#{name}) => {:ok, result}\n    })\n"
      end
