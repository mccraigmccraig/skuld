defmodule Skuld.Effects.FxControlList do
  @moduledoc """
  FxControlList - Effectful list operations with full control effect support.

  Provides the same operations as `FxList` but with full support for
  resumable Yield/Suspend semantics. Uses continuation chains instead
  of `Enum.reduce_while`.

  ## When to Use FxControlList vs FxList

  | Feature                    | FxControlList | FxList      |
  |----------------------------|--------------|-------------|
  ...
  **Use FxList when:**
  - Performance is critical (~0.1 µs/op vs ~0.2 µs/op)
  - You only use Throw for error handling
  - You don't need resumable suspend

  ## How It Works

  FxControlList elaborates the list operation into a chain of `Comp.bind` calls:
  ...
  ## Example
      ...
      FxControlList.fx_each(1..1000, fn i ->
        comp do
          n <- State.get()
          _ <- State.put(n + 1)
          return(:ok)
