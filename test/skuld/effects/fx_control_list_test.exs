defmodule Skuld.Effects.FxControlListTest do
  use ExUnit.Case, async: true

  alias Skuld.Comp
  alias Skuld.Effects.FxControlList
  ...
  describe "fx_map" do
    test "maps effectful function over empty list" do
      comp =
        FxControlList.fx_map([], fn x -> Comp.pure(x * 2) end)
    ...
    test "maps effectful function over list" do
      comp =
        FxControlList.fx_map([1, 2, 3], fn x -> Comp.pure(x * 2) end)
    ...
    test "maps with stateful effects" do
      comp =
        FxControlList.fx_map([1, 2, 3], fn x ->
    ...
    test "works with range" do
      comp =
        FxControlList.fx_map(1..3, fn x -> Comp.pure(x * 2) end)
  ...
  describe "fx_reduce" do
    test "reduces empty list to initial value" do
      comp =
        FxControlList.fx_reduce([], 0, fn x, acc -> Comp.pure(acc + x) end)
    ...
    test "reduces list with effectful function" do
      comp =
        FxControlList.fx_reduce([1, 2, 3, 4], 0, fn x, acc -> Comp.pure(acc + x) end)
    ...
    test "reduces with stateful effects" do
      comp =
        FxControlList.fx_reduce([1, 2, 3], [], fn x, acc ->
          Comp.bind(State.get(), fn count ->
            Comp.bind(State.put(count + 1), fn _ ->
  ...
  describe "fx_each" do
    test "executes effectful function for each element" do
      comp =
        Comp.bind(
          FxControlList.fx_each([1, 2, 3], fn x ->
    ...
    test "returns :ok" do
      comp =
        FxControlList.fx_each([1, 2, 3], fn _ -> Comp.pure(:ignored) end)
    ...
    test "works with range" do
      comp =
        Comp.bind(
          FxControlList.fx_each(1..5, fn _ ->
  ...
  describe "fx_filter" do
    test "filters empty list" do
      comp =
        FxControlList.fx_filter([], fn x -> Comp.pure(x > 0) end)
    ...
    test "filters list with pure predicate" do
      comp =
        FxControlList.fx_filter([1, 2, 3, 4, 5], fn x -> Comp.pure(rem(x, 2) == 0) end)
    ...
    test "filters with stateful predicate" do
      ...
      comp =
        FxControlList.fx_filter([1, 2, 3, 4, 5], fn _x ->
          Comp.bind(State.get(), fn count ->
            Comp.bind(State.put(count + 1), fn _ ->
  ...
  describe "fx_map with Throw" do
    test "throw in middle of list short-circuits" do
      comp =
        FxControlList.fx_map([1, 2, 3, 4, 5], fn x ->
          if x == 3 do
    ...
    test "try_catch returns ok on success" do
      comp =
        Throw.try_catch(FxControlList.fx_map([1, 2, 3], fn x -> Comp.pure(x * 2) end))
        |> Throw.with_handler()
  ...
  describe "fx_reduce with Throw" do
    test "throw in reducer short-circuits" do
      comp =
        FxControlList.fx_reduce([1, 2, 3, 4, 5], 0, fn x, acc ->
          if x == 3 do
  ...
  describe "fx_each with Throw" do
    test "throw in each short-circuits" do
      comp =
        Comp.bind(
          FxControlList.fx_each([1, 2, 3, 4, 5], fn x ->
            Comp.bind(State.get(), fn acc ->
  ...
  describe "fx_filter with Throw" do
    test "throw in predicate short-circuits" do
      comp =
        FxControlList.fx_filter([1, 2, 3, 4, 5], fn x ->
          if x == 3 do
            Throw.throw(:hit_three)
          else
            Comp.pure(rem(x, 2) == 0)
  ...
  describe "fx_reduce with Yield" do
    test "yield in reducer can be resumed" do
      comp =
        FxControlList.fx_reduce([1, 2, 3], 0, fn x, acc ->
  ...
  describe "fx_filter with Yield" do
    test "yield in predicate can be resumed" do
      comp =
        FxControlList.fx_filter([1, 2, 3, 4], fn x ->
