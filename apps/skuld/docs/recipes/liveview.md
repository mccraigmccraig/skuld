...
4. Use `pipe_event` to forward LiveView events as Yield resume values
...
`use AsyncPageMachine`; `pipe_event` generates `handle_event/3` clauses.
The module only needs `mount`, `pipe_event` calls, and `render`:
...
defmodule MyApp.CheckoutLive do
  ...
  AsyncPageMachine.pipe_event "submit_shipping", :runner, %{"address" => addr} do
    {:ok, %{address: addr}}
  end

  AsyncPageMachine.pipe_event "submit_payment", :runner, %{"payment" => payment} do
    {:ok, payment}
  end
...
| `AsyncPageMachine.pipe_event/2,4`| Generate `handle_event/3`     |
...
defmodule MyApp.CheckoutLive do
  ...
  PageMachine.pipe_event "submit_shipping", :checkout, %{"address" => a} do
    {:ok, %{address: a}}
  end

  PageMachine.pipe_event "submit_payment", :checkout, %{"payment" => p} do
    {:ok, p}
  end
