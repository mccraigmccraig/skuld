defmodule Mix.Tasks.Skuld.Docs.Nav do
  @moduledoc """
  ...
  def run(args) do
    ...
    {extras, groups, umbrella_home} = read_doc_config()
    titles = extract_titles(extras)

    if opts[:strip] do
      strip_all(extras)
    else
      nav_blocks = build_nav_blocks(extras, groups, titles, umbrella_home)
  ...
  defp read_doc_config do
    ...
    umbrella_home = Keyword.get(docs, :umbrella_home)
    {extras, groups, umbrella_home}
  end
  ...
  defp build_nav_blocks(extras, groups, titles, umbrella_home) do
    ...
    |> Map.new(fn {path, idx} ->
      ...
      nav =
        build_nav_line(path, prev, next, up_path, index_path, titles, group_name, umbrella_home)
  ...
  defp build_nav_line(
    ...
    # Umbrella (if configured)
    parts =
      if umbrella_home do
        parts ++ ["[Umbrella →](#{umbrella_home})"]
      else
        parts
      end
  ...
  defp check_all(extras, nav_blocks) do
    ...
    if outdated == [] do
      ...
      Mix.raise("Run `mix skuld.docs.nav` to update")
    end
  end
