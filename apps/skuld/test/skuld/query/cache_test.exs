defmodule Skuld.Query.CacheTest do
  ...
  defmodule CountingExecutor do
    ...
    @impl true
    def get_user(ops) do
      send(self(), {:executor_called, :get_user, length(ops)})
    ...
    def list_users(ops) do
      send(self(), {:executor_called, :list_users, length(ops)})
  ...
  defmodule CacheOptExecutor do
    ...
    @impl true
    def get_user(ops) do
      send(self(), {:executor_called, :cache_opt_get_user, length(ops)})
    ...
    def get_random(ops) do
      send(self(), {:executor_called, :get_random, length(ops)})
  ...
  defmodule OrderExecutor do
    ...
    def get_order(ops) do
      send(self(), {:executor_called, :get_order, length(ops)})
  ...
  defmodule FailingExecutor do
    ...
    @impl true
    def get_user(ops) do
      send(self(), {:executor_called, :failing_get_user, length(ops)})
      Skuld.Effects.Throw.throw(:executor_error)
    end

    @impl true
    def list_users(_ops), do: %{}
  end
