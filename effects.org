...
Your function signatures grow, and call sites get noisy.
...
- Developers can actually run tests locally
...
|-----------+--------------------+----------------------|
| Pure      | Regular functions, | =String.upcase/1=    |
|           | no effects         |                      |
| Effectful | Describes effects  | Your domain logic    |
|           | without doing them |                      |
| Handlers  | Actually perform   | Thin wrappers around |
|           | the I/O            | Repo, HTTP           |
...
* Debugging: Stacktraces Still Work

A common concern: "All this indirection - can I still debug?"

Yes. Skuld preserves original exceptions and stacktraces. When something breaks,
you see your code at the top of the stacktrace, not framework internals.

** Elixir exceptions point to your code
...
defmodule MyDomain do
  ...
  defcomp process(data) do
    # Line 5: this raise will appear in stacktrace
...
The stacktrace points to line 5 in your domain code - not to Skuld internals.

** Skuld's Throw effect

When you use =Throw.throw/1=, unhandled throws become =ThrowError= exceptions:
...
** Catching exceptions in computations

Elixir exceptions caught with =catch= arrive as maps with full context:

#+begin_src elixir
comp do
  raise ArgumentError, "oops"
catch
  {Throw, %{kind: :error, payload: ex, stacktrace: st}} ->
    {:caught, Exception.message(ex)}
end
|> Throw.with_handler()
|> Comp.run!()
...
The map contains =:kind= (error/throw/exit), =:payload= (the exception),
and =:stacktrace= (where it happened).

** The takeaway

Effects add a layer of indirection, but debugging remains practical:
- Exceptions show your source locations
- Stacktraces are preserved through the effect system
- You can catch and inspect errors with full context
...
#+begin_src elixir
defmodule BuggyCode do
  defcomp process(version) do
    _ <- State.put(10)
    _ <- case version do
...
- Effect logging for debugging/replay
- Serializable computation state and cold resume
- Coroutines and yield-based control flow
