...
- **`MyApp.Repository.EffectPort`** - effectful dispatch with:
  ...
  - **Bang** (when applicable) - `EffectPort.get_todo!(tenant_id, id)` unwrapping
...
### Writing a Behaviour implementation

Plain implementations satisfy the `.Behaviour` with plain Elixir
functions:
...
my_comp
|> Port.with_test_handler(%{
  MyApp.Repository.EffectPort.key(:get_todo, "tenant-1", "id-1") => {:ok, mock_todo}
})
|> Throw.with_handler()
|> Comp.run!()
```
...
- `EffectPort.key/N` helpers replace verbose `Port.key(Module, :name, [args...])` calls
...
- **Plain (outbound)** - effectful code emits Port effects (via
  `EffectPort`), resolved by `Port.with_handler/2` dispatching to a
...
test "effectful impl uses Port effect" do
  result =
    ...
    |> Port.with_test_handler(%{
      UserQueries.EffectPort.key(:get_user, "user-123") => {:ok, %User{id: "user-123"}}
    })
    |> Throw.with_handler()
    |> Comp.run!()
