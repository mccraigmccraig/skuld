...
- `@callback` declarations on the contract module itself — the contract IS the behaviour
...
- **Effectful facade**: `use Skuld.Effects.Port.Facade, contract: MyContract`
...
defmodule MyApp.Orders do
  use Skuld.Effects.Port.Facade, contract: MyApp.Orders.Contract
end
```

This generates `@callback` declarations on `MyApp.Orders.Contract` (making
it a behaviour), `MyApp.Orders.Contract.Effectful` (effectful behaviour),
...
|> Port.with_handler(%{
  MyApp.Orders.Contract => MyApp.OrderService.Effectful,
...
# Wiring
|> Port.with_handler(%{MyApp.Orders.Contract => MyApp.OrderService})
```
...
|> Port.with_handler(%{MyApp.Orders.Contract => MyApp.OrderService.Effectful})
|> Throw.with_handler()
```
