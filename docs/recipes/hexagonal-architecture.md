...
|---|--------------|----------------|----------------------------------------------------|
| 1 | Plain Elixir | Plain Elixir   | `MyContract.Port` (HexPort config dispatch)        |
...
The contract generates four submodules:

- `MyContract.Behaviour` — plain Elixir callbacks (generated by HexPort)
- `MyContract.Port` — plain dispatch facade via config/test handlers (generated by HexPort)
- `MyContract.Effectful` — computation-returning callbacks (generated by Skuld)
- `MyContract.EffectPort` — effectful dispatch facade with typed callers, bangs, and key helpers (generated by Skuld)
...
This generates `MyApp.Orders.Behaviour` and `MyApp.Orders.Effectful`
behaviours, `MyApp.Orders.Port` (plain dispatch facade), and
`MyApp.Orders.EffectPort` (effectful dispatch facade with caller
functions, bang variants, and key helpers).
...
### Step 2: Wire plain→plain with HexPort's `X.Port`

The existing implementations already have the right function signatures.
Declare that they satisfy the `Behaviour` and call through the generated
`Port` module:

```elixir
# Existing implementations — add @behaviour
defmodule MyApp.OrderService do
  @behaviour MyApp.Orders.Behaviour
  # ... existing code unchanged ...
end

defmodule MyApp.InventoryService do
  @behaviour MyApp.Inventory.Behaviour
  # ... existing code unchanged ...
end
```
...
```elixir
# config/config.exs
config :my_app, MyApp.Orders, MyApp.OrderService
config :my_app, MyApp.Inventory, MyApp.InventoryService
```

Now update callers to go through the generated `Port` modules:
...
defmodule MyApp.OrderService.Effectful do
  ...
  defcomp place_order(params) do
    reservation <- MyApp.Inventory.EffectPort.reserve_stock!(params.sku, params.qty)
...
OrderController → MyApp.Orders.Adapter [Effectful]
                  ...
                  OrderService.Effectful
                       ↓ Port effect (via Inventory.EffectPort)
...
effect (via `Inventory.EffectPort`), which the adapter's stack resolves
...
best left as plain Elixir behind a `Behaviour`. The port boundary
...
Both caller and implementation are plain Elixir. The generated `Port`
module dispatches to a config-resolved implementation through the
contract boundary.
...
MyApp.Orders.Port.place_order(params)
```
...
```elixir
# In effectful code (via EffectPort)
order <- MyApp.Orders.EffectPort.place_order!(params)
...
|> Port.with_test_handler(%{
  MyApp.Orders.EffectPort.key(:place_order, params) => {:ok, %Order{}}
})
|> Throw.with_handler()
|> Comp.run!()
...
comp
|> Port.with_test_handler(%{
  MyApp.Inventory.EffectPort.key(:reserve_stock, sku, qty) => {:ok, %Reservation{}}
})
|> Port.with_handler(%{MyApp.Orders => MyApp.OrderService.Effectful})
|> Throw.with_handler()
|> Comp.run!()
...
assert {:ok, %Order{}} = MyApp.Orders.Port.place_order(params)
...
The generated `Port` module dispatches to a config-resolved
implementation, so swapping in a Mox mock is just a config change.
...
Mox.defmock(MyApp.Repository.Mock, for: MyApp.Repository.Behaviour)
```

3. Point the `Port` module at the mock in test config:
...
defmodule MyApp.OrderService do
  ...
  def place_order(params) do
    item = Repository.Port.get!(Item, params.item_id)
    changeset = Order.changeset(%Order{}, %{item_id: item.id, qty: params.qty})
    Repository.Port.insert(changeset)
  end
end
```
...
2. **Use the generated `Port` module** — config-dispatched plain
   dispatch facade with your existing module as the default
3. **Use Mox in tests** — `Mox.defmock(Mock, for: MyContract.Behaviour)`,
   point `Port` at mock via `config/test.exs`
4. **Later, optionally** — use `Adapter.Effectful` for an effectful
   implementation and `EffectPort` for effectful callers
...
- Keep Behaviour implementations thin — just infrastructure calls
- The in-memory Behaviour implementation is your test double — no mocks needed
- Start with the generated `Port` module to impose boundaries, convert later
