...
|---|--------------|----------------|----------------------------------------------------|
| 1 | Plain Elixir | Plain Elixir   | `MyContract.Port` (app-defined via `use HexPort.Port`) |
...
The contract generates three submodules plus introspection:
...
- `MyContract.__port_operations__/0` — introspection (generated by HexPort)
...
separately with `use HexPort.Port, contract: MyContract, otp_app: :my_app`.
...
behaviours, and `MyApp.Orders.EffectPort` (effectful dispatch facade with
caller functions, bang variants, and key helpers).
...
### Step 2: Wire plain→plain with `HexPort.Port`

The existing implementations already have the right function signatures.
Declare that they satisfy the `Behaviour` and define Port modules for
plain dispatch:
...
defmodule MyApp.Orders.Port do
  use HexPort.Port, contract: MyApp.Orders, otp_app: :my_app
end

defmodule MyApp.Inventory.Port do
  use HexPort.Port, contract: MyApp.Inventory, otp_app: :my_app
end
```
...
module (via `use HexPort.Port`) dispatches to a config-resolved
...
2. **Define a Port module** — `use HexPort.Port, contract: MyContract, otp_app: :my_app`
...
- Start with a `HexPort.Port` module to impose boundaries, convert later
